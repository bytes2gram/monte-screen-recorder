 ...
 */
package org.bytes2gram.monte.media.math;
...
/**
 * Represents a TIFF RATIONAL number. <p> Two LONGs 32-bit (4-byte) unsigned integer: the first
 * represents the numerator of a fraction; the second, the denominator. </p> <p> Invariants: </p>
 * <ul> <li>denominator>=0, the denominator is always a positive integer</li> <li>0/1 is the unique
 * representation of 0.</li> <li>1/0,-1/0 are the unique representations of infinity.</li> </ul>
 *
...
public class Rational extends Number {
  ...
  private Rational(BigInteger numerator, BigInteger denominator, boolean reduceFraction) {
    ...
    if (denominator.equals(BigInteger.ZERO)) {
      // Invariant: 1/0, -1/0 are unique representations of infinity
      numerator =
          (numerator.compareTo(BigInteger.ZERO) > 0) ? BigInteger.ONE : BigInteger.ONE.negate();
  ...
  /**
   * Iteratively computes rational from double. <p>Reference:<br> <a href="http://www2.fz-juelich.de/video/cpp/html/exercises/exercise/Rational_cpp.html">
  ...
  private static Rational toRational(double x, double limit, int iterations) {
    ...
    if (d > limit || iterations == 0) {
      ...
      return valueOf(left, 1, false)
          .add(toRational(d, limit * 0.1, iterations - 1).inverse(), false);
    }
  }
   ...
   * Parses a string.
   * <p>
   * A rational can be represented in the following ways: <li>As a long number</li> <li>As a double
   * number</li> <li>As an integer/integer rational number</li>
   *
   ...
   * <p>
   * private Rational addAssign(Rational that) { if (this.den == that.den) { // => same denominator:
   * add numerators this.num += that.num; return this; }
   * <p>
   * // FIXME - handle overflow long s = scm(this.den, that.den); this.num = this.num * (s /
   * this.den) + that.num * (s / that.den); this.den = s;
   * <p>
   * <p>
   * return reduceAssign(); }
   */
  ...
  /**
   * Returns the closest rational with the specified denominator which is smaller or equal than this
   * number.
   */
  ...
  /**
   * Returns the closest rational with the specified denominator which is greater or equal than this
   * number.
   */
