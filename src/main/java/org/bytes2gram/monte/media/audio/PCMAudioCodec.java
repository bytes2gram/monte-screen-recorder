 ...
 */
package org.bytes2gram.monte.media.audio;
...
import static org.bytes2gram.monte.media.AudioFormatKeys.MIME_JAVA;
...
import static org.bytes2gram.monte.media.AudioFormatKeys.toAudioFormat;
...
import org.bytes2gram.monte.media.AbstractAudioCodec;
import org.bytes2gram.monte.media.Buffer;
import org.bytes2gram.monte.media.Format;
import org.bytes2gram.monte.media.io.ByteArrayImageInputStream;
import org.bytes2gram.monte.media.io.ByteArrayImageOutputStream;

/**
 * {@code PCMAudioCodec} performs sign conversion, endian conversion and quantization conversion of
 * PCM audio data. <p> Does not perform sampling rate conversion or channel conversion. <p> FIXME
 * Maybe create separate subclasses for AVI PCM and QuickTime PCM.
 *
...
public class PCMAudioCodec extends AbstractAudioCodec {
  ...
  public int process(Buffer in, Buffer out) {
    ...
    if (inFormat.get(SampleRateKey) == null || !inFormat.get(SampleRateKey)
        .equals(outFormat.get(SampleRateKey))) {
      out.setFlag(DISCARD);
      return CODEC_FAILED;
    ...
    if (toAudioFormat(inFormat).matches(toAudioFormat(outFormat))) {
      ...
      ByteArrayImageInputStream inStream = new ByteArrayImageInputStream(inData, in.offset,
          in.length, inOrder);
          ...
          case (16 << 16) | 8:
            if (inSigned == outSigned) {
              throw new UnsupportedOperationException(
                  "Unsupported sample size. in:" + inFormat + ", out:" + outFormat);
            } else if (inSigned) {
  ...
  /**
   * 16-bit audio data in and out. No sign conversion. Endian conversion is performed by the stream
   * objects.
   */
  ...
  /**
   * 16-bit audio data unsigned in and signed out. Endian conversion is performed by the stream
   * objects.
   */
  ...
  /**
   * 16-bit audio data signed in and unsigned out. Endian conversion is performed by the stream
   * objects.
   */
  ...
  /**
   * Signed 16-bit audio data in, unsigned 8-bit out. Endian conversion is performed by the stream
   * objects.
   */
   ...
   */
  protected void write8UTo8S(ImageInputStream in, ImageOutputStream out, boolean fixSilenceBug)
      throws IOException {
    try {
      byte[] buf = new byte[512];
      while (true) {
   ...
   */
  protected void write8STo8U(ImageInputStream in, ImageOutputStream out, boolean fixSilenceBug)
      throws IOException {
    try {
      byte[] buf = new byte[512];
      while (true) {
   ...
   */
  protected void write8STo8S(ImageInputStream in, ImageOutputStream out, boolean fixSilenceBug)
      throws IOException {
    try {
      byte[] buf = new byte[512];
      while (true) {
