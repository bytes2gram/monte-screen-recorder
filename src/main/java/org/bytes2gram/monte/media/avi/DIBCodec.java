...
/**
 * {@code DIBCodec} encodes a BufferedImage as a Microsoft Device Independent Bitmap (DIB) into a
 * byte array.
 * <p>
 * The DIB codec only works with the AVI file format. Other file formats, such as QuickTime, use a
 * different encoding for uncompressed video.
 * <p>
 ...
 * <p>
 * The pixels of a frame are written row by row from bottom to top and from the left to the right.
 * 24-bit pixels are encoded as BGR.
 * <p>
...
public class DIBCodec extends AbstractVideoCodec {
   ...
   */
  public void writeKey4(OutputStream out, byte[] pixels, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void writeKey8(OutputStream out, byte[] pixels, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void writeKey24(OutputStream out, int[] pixels, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
    int w3 = width * 3;
    byte[] bytes = new byte[w3]; // holds a scanline of raw image data with 3 channels of 8 bit data
    for (int xy = (height - 1) * scanlineStride + offset; xy >= offset;
        xy -= scanlineStride) { // Upside down
