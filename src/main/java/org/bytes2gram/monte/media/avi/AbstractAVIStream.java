 ...
 */
package org.bytes2gram.monte.media.avi;
...
import org.bytes2gram.monte.media.Buffer;
import org.bytes2gram.monte.media.Codec;
import org.bytes2gram.monte.media.Format;
import org.bytes2gram.monte.media.FormatKeys;
import org.bytes2gram.monte.media.VideoFormatKeys;
import org.bytes2gram.monte.media.riff.RIFFChunk;
...
public abstract class AbstractAVIStream {
  ...
  /**
   * Indicates that application should use the index, rather than the physical ordering of the
   * chunks in the file, to determine the order of presentation of the data. For example, this flag
   * could be used to create a list of frames for editing.
   */
   ...
   * Applications should warn the user before writing over a file with this flag set because the
   * user probably defragmented this file.
   */
  ...
  /**
   * Indicates this video stream contains palette changes. This flag warns the playback software
   * that it will need to animate the palette.
   */
  ...
  /**
   * The offset in the underlying ImageOutputStream. Normally this is 0 unless the underlying stream
   * already contained data when it was passed to the constructor.
   */
  ...
  protected static int typeToInt(String str) {
    int value =
        ((str.charAt(0) & 0xff) << 24) | ((str.charAt(1) & 0xff) << 16) | ((str.charAt(2) & 0xff)
            << 8) | (str.charAt(3) & 0xff);
    return value;
  }
  ...
  /**
   * Gets the position relative to the beginning of the QuickTime stream. <p> Usually this value is
   * equal to the stream position of the underlying ImageOutputStream, but can be larger if the
   * underlying stream already contained data.
   *
  ...
  /**
   * Seeks relative to the beginning of the AVI stream. <p> Usually this equal to seeking in the
   * underlying ImageOutputStream, but can be different if the underlying stream already contained
   * data.
   */
  ...
  /**
   * AVI stores media data in sample chunks. A sample chunk may contain one or more media samples. A
   * media sample is a single element in a sequence of time-ordered data.
   */
  protected static class Sample {
     ...
     *
     * @param duration The number of media samples contained in the sample chunk.
  ...
  protected static class MainHeader {
    ...
    /**
     * Specifies the number of microseconds (=10E-6 seconds) between frames. This value indicates
  ...
  /**
   * Represents a track (or "stream") in an AVI file. <p> A track is defined by an "strh" chunk,
   * which contains an {@code AVISTREAMHEADER} struct. Additional chunks can be provided depending
   * on the media type of the track. <p> See <a href="http://msdn.microsoft.com/en-us/library/ms779638(VS.85).aspx">
  ...
  protected abstract class Track {
    ...
    /**
     * {@code mediaType.fccType} contains a FOURCC that specifies the type of the data contained in
     * the stream. The following standard AVI values for video and audio are defined.
     * <p>
     * FOURCC	Description 'auds'	Audio stream 'mids'	MIDI stream 'txts'	Text stream 'vids'	Video
     * stream
     */
    ...
    /**
     * Interval between sync samples (keyframes). 0 = automatic. 1 = write all samples as sync
     * samples. n = sync every n-th sample.
     */
    ...
    /**
     * Optionally, contains a FOURCC that identifies a specific data handler. The data handler is
     * the preferred handler for the stream. For audio and video streams, this specifies the codec
     * for decoding the stream.
     */
    protected int fccHandler;
    /**
     * Contains any flags for the data stream. The bits in the high-order word of these flags are
     * specific to the type of data contained in the stream. The following standard flags are
     * defined.
     * <p>
     * Value	Description
     * <p>
     ...
     * <p>
     * AVISF_VIDEO_PALCHANGES 0x00010000 Indicates this video stream contains palette changes. This
     * flag warns the playback software that it will need to animate the palette.
     */
    protected int flags;
    /**
     * Specifies priority of a stream type. For example, in a file with multiple audio streams, the
     * one with the highest priority might be the default stream.
     */
     ...
     * For more information, see the remarks for the initialFrames member of the AVIMAINHEADER
     * structure.
     */
    protected long initialFrames = 0;
    /**
     * Used with rate to specify the time scale that this stream will use. Dividing rate by scale
     * gives the number of samples per second. For video streams, this is the frame rate. For audio
     * streams, this rate corresponds to the time needed to play blockAlign bytes of audio, which
    ...
    /**
     * Specifies the starting time for this stream. The units are defined by the rate and scale
     * members in the main file header. Usually, this is zero, but it can specify a delay time for a
     * stream that does not startTime concurrently with the file.
     */
    protected long startTime = 0;
    /**
     * Specifies the length of this stream. The units are defined by the rate and scale members of
     * the stream's header.
     */
    ...
    /**
     * Specifies an indicator of the quality of the data in the stream. Quality is represented as a
     * number between 0 and 10,000. For compressed data, this typically represents the value of the
     * quality parameter passed to the compression software. If set to –1, drivers use the default
     * quality value.
     */
    ...
    /**
     * The input buffer is used when one of the convenience methods without a Buffer parameter is
     * used.
     */
     ...
     * specified by the dwWidth and dwHeight members of the AVI main header structure. The rcFrame
     * member is typically used in support of multiple video streams. Set this rectangle to the
     * coordinates corresponding to the movie rectangle to update the whole movie rectangle. Units
     * for this member are pixels. The upper-left corner of the destination rectangle is relative to
    ...
    public void addSample(Sample s) {
      if (!samples.isEmpty()) {
        s.timeStamp =
            samples.get(samples.size() - 1).timeStamp + samples.get(samples.size() - 1).duration;
      }
      samples.add(s);
      length++;
    }
  ...
  /**
   * Represents a video track in an AVI file. <p> The format of a video track is defined in a "strf"
   * chunk, which contains a {@code BITMAPINFOHEADER} struct.
   *
   * </pre> //---------------------- // AVI Bitmap Info Header //
   * ---------------------- typedef struct { BYTE blue; BYTE green; BYTE red; BYTE reserved; }
   * RGBQUAD;
   * <p>
   * // Values for this enum taken from: // http://www.fourcc.org/index.php?http%3A//www.fourcc.org/rgb.php
   * enum { BI_RGB = 0x00000000, RGB = 0x32424752, // Alias for BI_RGB BI_RLE8 = 0x01000000, RLE8 =
   * 0x38454C52, // Alias for BI_RLE8 BI_RLE4 = 0x00000002, RLE4 = 0x34454C52, // Alias for BI_RLE4
   * BI_BITFIELDS = 0x00000003, raw = 0x32776173, RGBA = 0x41424752, RGBT = 0x54424752, cvid =
   * "cvid" } bitmapCompression;
   * <p>
   * typedef struct { DWORD structSize; // Specifies the number of bytes required by the structure.
   * LONG width; // Specifies the width of the bitmap. // - For RGB formats, the width is specified
   * in pixels. // - The same is true for YUV formats if the bitdepth is an even power // of 2. // -
   * For YUV formats where the bitdepth is not an even power of 2, // however, the width is
   * specified in bytes. // Decoders and video sources should propose formats where "width" is //
   * the width of the image. If the video renderer is using DirectDraw, it // modifies the format so
   * that "width" equals the stride of the surface, // and the "target" member of the
   * VIDEOINFOHEADER or VIDEOINFOHEADER2 // structure specifies the image width. Then it proposes
   * the modified // format using IPin::QueryAccept. // For RGB and even-power-of-2 YUV formats, if
   * the video renderer does // not specify the stride, then round the width up to the nearst DWORD
   * // boundary to find the stride. LONG height; // Specifies the height of the bitmap, in pixels.
   * // - For uncompressed RGB bitmaps, if "height" is positive, the bitmap // is a bottom-up DIB
   * with the origin at the lower left corner. If // "height" is negative, the bitmap is a top-down
   * DIB with the origin // at the upper left corner. // - For YUV bitmaps, the bitmap is always
   * top-down, regardless of the // sign of "height". Decoders should offer YUV formats with postive
   * // "height", but for backward compatibility they should accept YUV // formats with either
   * positive or negative "height". // - For compressed formats, height must be positive, regardless
   * of // image orientation. WORD planes; // Specifies the number of planes for the target device.
   * This value must // be set to 1. WORD bitCount; // Specifies the number of bits per pixel.
   * //DWORD enum bitmapCompression compression; FOURCC enum bitmapCompression compression; // If
   * the bitmap is compressed, this member is a FOURCC the specifies // the compression. // Value
   * Description // BI_RLE8 A run-length encoded (RLE) format for bitmaps with 8 // bpp. The
   * compression format is a 2-byte format // consisting of a count byte followed by a byte
   * containing a color index. For more information, see Bitmap Compression. // BI_RLE4 An RLE
   * format for bitmaps with 4 bpp. The compression // format is a 2-byte format consisting of a
   * count byte // followed by two word-length color indexes. For more // information, see Bitmap
   * Compression. // BI_JPEG Windows 98/Me, Windows 2000/XP: Indicates that the // image is a JPEG
   * image. // BI_PNG Windows 98/Me, Windows 2000/XP: Indicates that the // image is a PNG image. //
   * For uncompressed formats, the following values are possible: // Value Description // BI_RGB
   * Uncompressed RGB. // BI_BITFIELDS Specifies that the bitmap is not compressed and that // the
   * color table consists of three DWORD color masks // that specify the red, green, and blue
   * components, // respectively, of each pixel. This is valid when used // with 16- and 32-bpp
   * bitmaps. DWORD imageSizeInBytes; // Specifies the size, in bytes, of the image. This can be set
   * to 0 for // uncompressed RGB bitmaps. LONG xPelsPerMeter; // Specifies the horizontal
   * resolution, in pixels per meter, of the // target device for the bitmap. LONG yPelsPerMeter; //
   * Specifies the vertical resolution, in pixels per meter, of the target // device for the bitmap.
   * DWORD numberOfColorsUsed; // Specifies the number of color indices in the color table that are
   * // actually used by the bitmap DWORD numberOfColorsImportant; // Specifies the number of color
   * indices that are considered important // for displaying the bitmap. If this value is zero, all
   * colors are // important. RGBQUAD colors[]; // If the bitmap is 8-bpp or less, the bitmap uses a
   * color table, which // immediately follows the BITMAPINFOHEADER. The color table consists of //
   * an array of RGBQUAD values. The size of the array is given by the // "clrUsed" member. If
   * "clrUsed" is zero, the array contains the // maximum number of colors for the given bitdepth;
   * that is, // 2^"bitCount" colors. } BITMAPINFOHEADER;
   * </pre>
   */
  protected class VideoTrack extends Track {
     ...
     * <p>
     * For uncompressed RGB bitmaps, if height is positive, the bitmap is a bottom-up DIB with the
     * origin at the lower left corner. If height is negative, the bitmap is a top-down DIB with the
     * origin at the upper left corner. For YUV bitmaps, the bitmap is always top-down, regardless
     * of the sign of height. Decoders should offer YUV formats with positive height, but for
     * backward compatibility they should accept YUV formats with either positive or negative
     * height. For compressed formats, height must be positive, regardless of image orientation.
     */
    int height;
    /**
     ...
     * average number of bits per pixel. For compressed formats, this value is the implied bit depth
     ...
     * little-endian order. For example, YUYV video has the FOURCC 'VYUY' or 0x56595559.
     * <p>
     ...
     * <p>
     * For 16-bpp bitmaps, if compression equals BI_RGB, the format is always RGB 555. If
     * compression equals BI_BITFIELDS, the format is either RGB 555 or RGB 565. Use the subtype
     * GUID in the AM_MEDIA_TYPE structure to determine the specific RGB type.
     */
    String compression;
    /**
     * Specifies the size, in bytes, of the image. This can be set to 0 for uncompressed RGB
     * bitmaps.
     */
    long sizeImage;
    /**
     ...
     * bitmap. See Remarks for more information.
     */
    long clrUsed;
    /**
     * Specifies the number of color indices that are considered important for displaying the
     * bitmap. If this value is zero, all colors are important.
     */
    long clrImportant;
    ...
    public VideoTrack(int trackIndex, int fourCC, Format videoFormat) {
      ...
      sampleChunkFourCC = videoFormat != null && videoFormat.get(FormatKeys.EncodingKey).equals(
          VideoFormatKeys.ENCODING_AVI_DIB) ? twoCC | DB_ID : twoCC | DC_ID;
    }
  ...
  protected class AudioTrack extends Track {
    ...
    /**
     * Microsoft Corporation IEEE754: range (+1, -1] 32-bit/64-bit format as defined by MSVC++
     * float/double type
     */
    ...
    /**
     * the WAVE_FORMAT_DEVELOPMENT format tag can be used during the development phase of a new wave
     * format. Before shipping, you MUST acquire an official format tag from Microsoft.
     */
     ...
     * compression algorithms. A complete list of format tags can be found in the Mmreg.h header
     * file. For one- or two-channel Pulse Code Modulation (PCM) data, this value should be
     * WAVE_FORMAT_PCM=0x0001.
     * <p>
     * If wFormatTag equals WAVE_FORMAT_EXTENSIBLE=0xFFFE, the structure is interpreted as a
     * WAVEFORMATEXTENSIBLE structure. If wFormatTag equals WAVE_FORMAT_MPEG, the structure is
     * interpreted as an MPEG1WAVEFORMAT structure. If wFormatTag equals MPEGLAYER3WAVEFORMAT, the
     * structure is interpreted as an MPEGLAYER3WAVEFORMAT structure. Before reinterpreting a
     * WAVEFORMATEX structure as one of these extended structures, verify that the actual structure
     * size is sufficiently large and that the cbSize member indicates a valid size.
     */
     ...
     * values for samplesPerSec are 8.0 kHz, 11.025 kHz, 22.05 kHz, and 44.1 kHz. For non-PCM
     * formats, this member must be computed according to the manufacturer's specification of the
     * format tag.
     */
    protected long samplesPerSec;
    /**
     * Required average data-transfer rate, in bytes per second, for the format tag. If wFormatTag
     * is WAVE_FORMAT_PCM, avgBytesPerSec should be equal to the product of samplesPerSec and
     * blockAlign. For non-PCM formats, this member must be computed according to the manufacturer's
     * specification of the format tag.
     */
    protected long avgBytesPerSec;
    /**
     * Block alignment, in bytes. The block alignment is the minimum atomic unit of data for the
     * wFormatTag format type. If wFormatTag is WAVE_FORMAT_PCM or WAVE_FORMAT_EXTENSIBLE,
     * blockAlign must be equal to the product of channels and bitsPerSample divided by 8 (bits per
     * byte). For non-PCM formats, this member must be computed according to the manufacturer's
     * specification of the format tag.
     * <p>
     * Software must process a multiple of blockAlign bytes of data at a time. Data written to and
     * read from a device must always startTime at the beginning of a block. For example, it is
     * illegal to startTime playback of PCM data in the middle of a sample (that is, on a
     ...
     * bitsPerSample should be equal to 8 or 16. For non-PCM formats, this member must be set
     * according to the manufacturer's specification of the format tag. If wFormatTag is
     * WAVE_FORMAT_EXTENSIBLE, this value can be any integer multiple of 8. Some compression schemes
     * cannot define a value for bitsPerSample, so this member can be zero.
     */
  ...
  protected class CompositeChunk extends Chunk {
    ...
    /**
     * Writes the chunk and all its children to the ImageOutputStream and disposes of all resources
     * held by the chunk.
     *
