/**
 * @(#)AVIOutputStream.java Copyright (c) 2011-2012 Werner Randelshofer, Goldau, Switzerland. All
 * rights reserved.
 * <p>
...
import static org.bytes2gram.monte.media.FormatKeys.MIME_AVI;
...
public class AVIOutputStream extends AbstractAVIStream {
   ...
   * than 1.
   */
  public int addVideoTrack(String fccHandler, long scale, long rate, int width, int height,
      int depth, int syncInterval) throws IOException {
    ensureFinished();
   ...
   */
  public void writePalette(int track, byte[] data, int off, int len, boolean isKeyframe)
      throws IOException {
    Track tr = tracks.get(track);
    if (!(tr instanceof VideoTrack)) {
   ...
   * @throws IOException if writing the sample data failed.
   */
  public void writeSample(int track, byte[] data, int off, int len, boolean isKeyframe)
      throws IOException {
    ensureStarted();
    Track tr = tracks.get(track);
    ...
    if (!isKeyframe && tr.samples.isEmpty()) {
      throw new IllegalStateException(
          "The first sample in a track must be a keyframe.\nTrack=" + track + ", " + tr.format);
    }
    ...
    if (isKeyframe && 0 != (tr.flags & STRH_FLAG_VIDEO_PALETTE_CHANGES)) {
      throw new IllegalStateException(
          "Only palette changes can be marked as keyframe.\nTrack=" + track + ", " + tr.format);
    }
  ...
  public void writeSamples(int track, int sampleCount, byte[] data, int off, int len,
      boolean isKeyframe) throws IOException {
    ensureStarted();
    Track tr = tracks.get(track);
    if (tr.mediaType == AVIMediaType.AUDIO) {
      ...
      long length = getRelativeStreamPosition() - offset;
      dc.finish();
      Sample s = new Sample(dc.chunkType, sampleCount, offset, length,
          isKeyframe | tr.samples.isEmpty());
      tr.addSample(s);
      idx1.add(s);
  ...
  private void writeEpilog() throws IOException {
    ...
    for (Track tr : tracks) {
      ...
      d.writeInt(
          (int) tr.initialFrames); // initialFrames: how far audio data is ahead of the video frames
      ...
      d.writeInt(tr instanceof AudioTrack ? ((AudioTrack) tr).blockAlign
          : (int) dwSampleSize); // dwSampleSize
