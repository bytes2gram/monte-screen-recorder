/**
 * @(#)AVIWriter.java Copyright (c) 2011 Werner Randelshofer, Goldau, Switzerland. All rights
 * reserved.
 * <p>
 * You may not use, copy or modify this file, except in compliance onlyWith the license agreement
 * you entered into onlyWith Werner Randelshofer. For details see accompanying license terms.
 */
package org.bytes2gram.monte.media.avi;

import static org.bytes2gram.monte.media.AudioFormatKeys.ChannelsKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_ALAW;
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_MP3;
...
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_ULAW;
...
import org.bytes2gram.monte.media.riff.RIFFParser;
...
public class AVIWriter extends AVIOutputStream implements MovieWriter {
  ...
  public final static Format VIDEO_SCREEN_CAPTURE = new Format(
      MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI,
      EncodingKey, ENCODING_AVI_TECHSMITH_SCREEN_CAPTURE, CompressorNameKey,
  ...
  public void write(int track, BufferedImage image, long duration) throws IOException {
    ...
    // Encode pixel data
    {
      ...
      writeSample(track, (byte[]) vt.outputBuffer.data, vt.outputBuffer.offset,
          vt.outputBuffer.length, isKeyframe && !paletteChange);
            /*
  ...
  private boolean writePalette(int track, BufferedImage image, boolean isKeyframe)
      throws IOException {
  ...
  private boolean writePalette(int track, IndexColorModel imgPalette, boolean isKeyframe)
      throws IOException {
    ensureStarted();
      ...
      if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
        ...
        if (null == tr.codec.setOutputFormat(
          ...
          throw new UnsupportedOperationException(
              "Track " + tr + " codec does not support format " + fmt + ". codec=" + tr.codec);
        }
      } else {
        ...
        if (null == tr.codec.setOutputFormat(
            fmt.prepend(FixedFrameRateKey, true,
                ...
                DataClassKey, byte[].class))) {
          throw new UnsupportedOperationException(
              "Track " + tr + " codec " + tr.codec + " does not support format. " + fmt);
        }
      }
    }
  }
