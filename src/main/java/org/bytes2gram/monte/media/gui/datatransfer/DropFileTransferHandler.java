 ...
 */
package org.bytes2gram.monte.media.gui.datatransfer;
...
/**
 * The DropFileTransferHandler can be used to add drag and drop support. When a file is dropped, the
 * supplied ActionListner is invoked. The filename is passed in the action command.
 *
...
public class DropFileTransferHandler extends TransferHandler {
    ...
    if (t.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
      ...
      try {
        ...
        if (list.size() > 0) {
          ...
          actionListener
              .actionPerformed(new ActionEvent(this, ActionEvent.ACTION_PERFORMED, file.getPath()));
        }
        imported = true;
  ...
  /**
   * Try to find a flavor that can be used to import a Transferable. The set of usable flavors are
  ...
  /**
   * This is the type of transfer actions supported by the source.  Some models are not mutable, so
   * a transfer operation of COPY only should be advertised in that case.
   *
   ...
   * @return This is implemented to return NONE if the component is a JPasswordField since exporting
   * data via user gestures is not allowed.  If the text component is editable, COPY_OR_MOVE is
  ...
  /**
   * This method is called after data has been exported.  This method should remove the data that
   * was transfered if the action was MOVE.
   *
