...
public class CodecChain implements Codec {
  ...
  public int process(Buffer in, Buffer out) {
    ...
    return (secondState & (-1 ^ CODEC_INPUT_NOT_CONSUMED)) | (firstState & (-1
  ...
  public String reportElapsedTime() {
    if (second instanceof CodecChain) {
      ...
      return "{" + first.getName() + " " + firstElapsed + ", " + second.getName() + " "
          + secondElapsed + "}";
    }
  }
  ...
  private String reportElapsedTime0() {
    if (second instanceof CodecChain) {
      return ", " + first.getName() + " " + firstElapsed + ((CodecChain) second)
          .reportElapsedTime0();
    } else {
      return ", " + first.getName() + " " + firstElapsed + ", " + second.getName() + " "
          + secondElapsed;
