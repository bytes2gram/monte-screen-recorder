...
public class RIFFPrimitivesInputStream extends FilterInputStream {
   ...
   * unsigned integer created by concatenating four ASCII characters. For example, the FOURCC 'abcd'
   * is represented on a Little-Endian system as 0x64636261. FOURCCs can contain space characters,
   * so ' abc' is a valid FOURCC. The AVI file format uses FOURCC codes to identify stream types,
   * data chunks, index entries, and other information."
   */
  ...
  public int readFully(byte[] b, int offset, int length)
    ...
    if (count != length) {
      throw new EOFException(
          "readFully for " + length + " bytes, unexpected EOF after " + count + " bytes.");
    }
  ...
  /**
   * Skips over and discards n bytes of data from this input stream. This skip method tries to skip
