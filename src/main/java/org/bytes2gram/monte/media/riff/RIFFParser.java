 ...
 */
package org.bytes2gram.monte.media.riff;
...
import org.bytes2gram.monte.media.AbortException;
import org.bytes2gram.monte.media.ParseException;
import org.bytes2gram.monte.media.io.ImageInputStreamAdapter;
 ...
 * </pre>
 * where 'RIFF' is the literal FOURCC code 'RIFF', fileSize is a 4-byte value giving the size of the
 * data in the file, and fileType is a FOURCC that identifies the specific file type. The value of
 * fileSize includes the size of the fileType FOURCC plus the size of the data that follows, but
 * does not include the size of the 'RIFF' FOURCC or the size of fileSize. The file data consists of
 * chunks and lists, in any order.
 * <p>
 * <b>FOURCCs</b><br>
 * A FOURCC (four-character code) is a 32-bit unsigned integer created by concatenating four ASCII
 * characters. For example, the FOURCC 'abcd' is represented on a Little-Endian system as
 * 0x64636261. FOURCCs can contain space characters, so ' abc' is a valid FOURCC. The RIFF file
 * format uses FOURCC codes to identify stream types, data chunks, index entries, and other
 * information.
 * <p>
 ...
 * ckID ckSize ckData
 * </pre>
 * where ckID is a FOURCC that identifies the data contained in the chunk, ckData is a 4-byte value
 * giving the size of the data in ckData, and ckData is zero or more bytes of data. The data is
 * always padded to nearest WORD boundary. ckSize gives the size of the valid data in the chunk; it
 * does not include the padding, the size of ckID, or the size of ckSize.
 * <p>
 * A list has the following form:
 * <pre>
 * 'LIST' listSize listType listData
 * </pre>
 * where 'LIST' is the literal FOURCC code 'LIST', listSize is a 4-byte value giving the size of the
 * list, listType is a FOURCC code, and listData consists of chunks or lists, in any order. The
 * value of listSize includes the size of listType plus the size of listData; it does not include
 * the 'LIST' FOURCC or the size of listSize.
 * <p>
 * For more information see: http://msdn.microsoft.com/archive/default.asp?url=/archive/en-us/directx9_c/directx/htm/avirifffilereference.asp
 ...
 * that does  something useful at each call to the visit method. Then create an instance of a
 * RIFFParser and invoke the #interpret method.
 *
 * <pre>
 ...
 * that  helps differentiate between data chunks, property chunks and collection chunks) a  reader
 * must set up the interpreter with some contextual information before starting the interpreter.
 * <p>
 * Once at least one chunk has been declared, the interpreter will only call the visitor for
 * occurences of the declared group chunks and data chunks. The property chunks and the collection
 * chunks can be obtained from the current group chunk by calling #getProperty or #getCollection.
 * <br>Note: All information the visitor can obtain during interpretation is only
 * valid during the actual #visit... call. Dont try to get information about properties or
...
public class RIFFParser extends Object {
   ...
   *
   * <p>Obligation
   * The visitor may throw an ParseException or an AbortException during tree traversal.
   *
   * @throws ParseException Is thrown when an interpretation error occured. The stream is positioned
   *                        where the error occured.
   * @exception AbortException Is thrown when the visitor decided to abort the interpretation.
   */
    ...
    if (id == RIFF_ID) {
      ...
      if (iin != null) {
        throw new ParseException(
            "Invalid RIFF File ID: \"" + idToString(id) + " 0x" + Integer.toHexString(id) + " near "
                + iin.getStreamPosition() + " 0x" + Long.toHexString(iin.getStreamPosition()));
      } else {
        throw new ParseException(
            "Invalid RIFF File ID: \"" + idToString(id) + " 0x" + Integer.toHexString(id));
      }
    }
  }
      ...
      while (getScan(in) < finish) {
        ...
        if (id == RIFF_ID) {
          ...
          ParseException pex = new ParseException(
              "Invalid Chunk: \"" + id + "\" at offset:" + idscan);
  ...
  protected boolean isDataChunk(RIFFChunk chunk) {
    if (dataChunks == null) {
      if (collectionChunks == null && propertyChunks == null && (stopChunkTypes == null
          || !stopChunkTypes.contains(chunk.getType()))) {
        return true;
      } else {
        return false;
      }
