 ...
 */
package org.bytes2gram.monte.media.io;
...
/**
 * This class implements an image output stream in which the data is written into a byte array. The
 * buffer automatically grows as data is written to it. The data can be retrieved using {@code
 * toByteArray()}, {@code toImageOutputStream()} and {@code toOutputStream()}.
 * <p>
 * Closing a {@code ByteArrayImageOutputStream} has no effect. The methods in this class can be
...
public class ByteArrayImageOutputStream extends ImageOutputStreamImpl {
   ...
   * element <code>buf[streamPos]</code> is the next byte to be read.
   */
  protected byte buf[];
  /**
   ...
   * <code>streamPos</code> equals <code>count</code>, then <code>-1</code> is returned to indicate
   * end of file. Otherwise, the  number <code>k</code> of bytes read is equal to the smaller of
   ...
   * by <code>System.arraycopy</code>. The value <code>k</code> is added into <code>streamPos</code>
   ...
   * equal to the smaller of <code>n</code> and  <code>count-streamPos</code>. The value
   * <code>k</code> is added into <code>streamPos</code> and <code>k</code> is returned.
   *
   ...
   * stream.
   * <p>
   * The value returned is <code>count&nbsp;- streamPos</code>, which is the number of bytes
