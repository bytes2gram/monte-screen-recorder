...
public class ImageInputStreamAdapter extends FilterInputStream {
   ...
   * same bytes.
   * <p>
   * The <code>readlimit</code> argument tells this input stream to allow that many bytes to be read
   ...
   * stream is of the type handled by the parse, it just chugs along happily. If the stream is not
   * of that type, the parser should toss an exception when it fails. If this happens within
  ...
  /**
   * Tests if this input stream supports the <code>mark</code> and <code>reset</code> methods. This
   * method simply performs <code>in.markSupported()</code>.
   *
