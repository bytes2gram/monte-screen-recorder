...
package org.bytes2gram.monte.media.image;
...
public class Images {
  ...
  public static Image createImage(Class baseClass, String location) {
    ...
    if (resource == null) {
      System.err.println(
          "Warning: Images.createImage no resource found for " + baseClass + " " + location);
      return null;
    }
  ...
  public static BufferedImage toIntImage(BufferedImage img) {
    if (img.getRaster().getDataBuffer() instanceof DataBufferInt) {
      ...
      BufferedImage intImg = new BufferedImage(img.getWidth(), img.getHeight(),
  ...
  /**
   * Converts an image into an array of integer pixels. If the image has an integer data buffer, the
   * internal pixel array is returned.
   */
