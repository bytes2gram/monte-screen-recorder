 ...
 */
package org.bytes2gram.monte.media.quicktime;

import static java.lang.Math.max;
import static org.bytes2gram.monte.media.AudioFormatKeys.ByteOrderKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.ChannelsKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.FrameSizeKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.SampleRateKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.SampleSizeInBitsKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.SignedKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.CompressorNameKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.DataClassKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.DepthKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.EncodingKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.FrameRateKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.HeightKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.MIME_QUICKTIME;
import static org.bytes2gram.monte.media.VideoFormatKeys.MediaType;
import static org.bytes2gram.monte.media.VideoFormatKeys.MediaTypeKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.MimeTypeKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.WidthKey;
...
import org.bytes2gram.monte.media.io.ImageOutputStreamAdapter;
import org.bytes2gram.monte.media.math.Rational;

/**
 * This class provides low-level support for writing already encoded audio and video samples into a
 * QuickTime file.
 *
...
public class QuickTimeOutputStream extends AbstractQuickTimeStream {
  ...
  /**
   * Sets the time scale for this movie, that is, the number of time units that pass per second in
   * its time coordinate system. <p> The default value is 600.
   *
   * @param timeScale
   */
   ...
   * <p> The returned value is the media duration of the track in the movies's time scale.
   *
   * @param track Track index.
  ...
  /**
   * Returns the track duration in the movie's time scale. <p> If the track has an edit-list, the
   * track duration is the sum of all edit durations.
   ...
   * @param icm   IndexColorModel. Specify null to use the standard Macintosh color table.
   */
  ...
  /**
   * Gets the preferred color table for displaying the movie on devices that support only 256
   * colors.
   *
   * @param track The track number.
   * @return The color table or null, if the video uses the standard Macintosh color table.
   */
  ...
  /**
   * Sets the edit list for the specified track. <p> In the absence of an edit list, the
   * presentation of the track starts immediately. An empty edit is used to offset the start time of
   * a track. <p>
   *
   ...
   * Adds a video track.
   *
   * @param compressionType The QuickTime "image compression format" 4-Character code. A list of
   *                        supported 4-Character codes is given in qtff, table 3-1, page 96.
   ...
   * @param syncInterval    Interval for sync-samples. 0=automatic. 1=all frames are keyframes.
   *                        Values larger than 1 specify that for every n-th frame is a keyframe.
   *                        Apple's QuickTime will not work properly if there is not at least one
   *                        keyframe every second.
   * @return Returns the track index.
   * @throws IllegalArgumentException if {@code width} or {@code height} is smaller than 1, if the
   *                                  length of {@code compressionType} is not equal to 4, if the
   *                                  length of the {@code compressorName} is not between 1 and 32,
   *                                  if the tiimeScale is not between 1 and 2^32.
   */
  public int addVideoTrack(String compressionType, String compressorName, long timeScale, int width,
      int height, int depth, int syncInterval) throws IOException {
    ensureStarted();
    if (compressionType == null || compressionType.length() != 4) {
      throw new IllegalArgumentException(
          "compressionType must be 4 characters long:" + compressionType);
    }
    if (compressorName == null || compressorName.length() < 1 || compressorName.length() > 32) {
      throw new IllegalArgumentException(
          "compressorName must be between 1 and 32 characters long:" + (compressorName == null
              ? "null" : "\"" + compressorName + "\""));
    }
    if (timeScale < 1 || timeScale > (2L << 32)) {
      throw new IllegalArgumentException("timeScale must be between 1 and 2^32:" + timeScale);
    ...
    if (width < 1 || height < 1) {
      throw new IllegalArgumentException(
          "Width and height must be greater than 0, width:" + width + " height:" + height);
    }
   ...
   * Adds an audio track.
   *
   * @param compressionType  The QuickTime 4-character code. A list of supported 4-Character codes
   *                         is given in qtff, table 3-7, page 113.
   * @param timeScale        The media time scale between 1 and 2^32.
   ...
   * @param frameDuration    The frame duration, expressed in the media’s timescale, where the
   *                         timescale is equal to the sample rate. For uncompressed formats, this
   *                         field is always 1.
   * @param frameSize        For uncompressed audio, the number of bytes in a sample for a single
   *                         channel (sampleSize divided by 8). For compressed audio, the number of
   *                         bytes in a frame.
   * @return Returns the track index.
   * @throws IllegalArgumentException if the audioFormat is not 4 characters long, if the time scale
   *                                  is not between 1 and 2^32, if the integer portion of the
   *                                  sampleRate is not equal to the timeScale, if numberOfChannels
   *                                  is not 1 or 2.
   */
  public int addAudioTrack(String compressionType, //
    ...
    if (compressionType == null || compressionType.length() != 4) {
      throw new IllegalArgumentException(
          "audioFormat must be 4 characters long:" + compressionType);
    }
    if (timeScale < 1 || timeScale > (2L << 32)) {
      throw new IllegalArgumentException("timeScale must be between 1 and 2^32:" + timeScale);
    }
    if (timeScale != (int) Math.floor(sampleRate)) {
      throw new IllegalArgumentException(
          "timeScale: " + timeScale + " must match integer portion of sampleRate: " + sampleRate);
    }
  ...
  /**
   * Sets the compression quality of a track. <p> A value of 0 stands for "high compression is
   * important" a value of 1 for "high image quality is important". <p> Changing this value affects
   * the encoding of video frames which are subsequently written into the track. Frames which have
   * already been written are not changed. <p> This value has no effect on videos encoded with
   * lossless encoders such as the PNG format. <p> The default value is 0.97.
   *
   * @param newValue
   */
   ...
   *
   * @param track The track number.
   * @param i     Interval between sync samples (keyframes). 0 = automatic. 1 = write all samples as
   *              sync samples. n = sync every n-th sample.
   */
  ...
  public void setMovieTransformationMatrix(double[] matrix) {
    if (matrix.length != 9) {
      throw new IllegalArgumentException(
          "matrix must have 9 elements, matrix.length=" + matrix.length);
    }
  ...
  /**
   * Sets the state of the QuickTimeWriter to started. <p> If the state is changed by this method,
   * the prolog is written.
   */
  ...
  /**
   * Writes an already encoded sample from a file into a track. <p> This method does not inspect the
   * contents of the samples. The contents has to match the format and dimensions of the media in
   * this track.
   *
   ...
   * @throws IllegalArgumentException  if the track does not support video, if the duration is less
   *                                   than 1, or if the dimension of the frame does not match the
  ...
  /**
   * Writes an already encoded sample from an input stream into a track. <p> This method does not
   * inspect the contents of the samples. The contents has to match the format and dimensions of the
   * media in this track.
   *
   * @param track    The track index.
  ...
  /**
   * Writes an already encoded sample from a byte array into a track. <p> This method does not
   * inspect the contents of the samples. The contents has to match the format and dimensions of the
   * media in this track.
   *
   * @param track    The track index.
  ...
  /**
   * Writes multiple sync samples from a byte array into a track. <p> This method does not inspect
   * the contents of the samples. The contents has to match the format and dimensions of the media
   * in this track.
   *
   * @param track          The track index.
   * @param sampleCount    The number of samples.
   * @param data           The encoded sample data. The length of data must be dividable by
   *                       sampleCount.
   * @param sampleDuration The duration of a sample. All samples must have the same duration.
   * @throws IllegalArgumentException if {@code sampleDuration} is less than 1 or if the length of
   *                                  {@code data} is not dividable by {@code sampleCount}.
  ...
  public void writeSamples(int track, int sampleCount, byte[] data, long sampleDuration,
      boolean isSync) throws IOException {
   ...
   * @throws IOException              if writing the sample data failed.
   */
  public void writeSamples(int track, int sampleCount, byte[] data, int off, int len,
      long sampleDuration) throws IOException {
    writeSamples(track, sampleCount, data, off, len, sampleDuration, true);
  }

  /**
   * Writes multiple samples from a byte array into a track. <p> This method does not inspect the
   * contents of the data. The contents has to match the format and dimensions of the media in this
   * track.
   *
   * @param track          The track index.
   * @param sampleCount    The number of samples.
   ...
   * @param len            The number of bytes to write. Must be dividable by sampleCount.
   * @param sampleDuration The duration of a sample. All samples must have the same duration.
   * @param isSync         Whether the samples are sync samples. All samples must either be sync
   *                       samples or non-sync samples.
   * @throws IllegalArgumentException if the duration is less than 1.
   * @throws IOException              if writing the sample data failed.
   */
  public void writeSamples(int track, int sampleCount, byte[] data, int off, int len,
      long sampleDuration, boolean isSync) throws IOException {
    ensureStarted();
    if (sampleDuration <= 0) {
      throw new IllegalArgumentException(
          "sampleDuration must be greater 0, sampleDuration=" + sampleDuration + " track=" + track);
    }
    if (sampleCount <= 0) {
      throw new IllegalArgumentException(
          "sampleCount must be greater 0, sampleCount=" + sampleCount + " track=" + track);
    }
    if (len % sampleCount != 0) {
      throw new IllegalArgumentException(
          "len must be divisable by sampleCount len=" + len + " sampleCount=" + sampleCount
              + " track=" + track);
    }
    Track t = tracks.get(track); // throws index out of bounds exception if illegal track index
    ensureOpen();
    ...
    Sample last = new Sample(sampleDuration, offset + sampleLength * (sampleCount - 1),
        sampleLength);
    t.addChunk(new Chunk(first, last, sampleCount, 1), isSync);
  }

  /**
   * Returns true if the limit for media samples has been reached. If this limit is reached, no more
   * samples should be added to the movie. <p> QuickTime files can be up to 64 TB long, but there
   * are other values that may overflow before this size is reached. This method returns true when
   * the files size exceeds 2^60 or when the media duration value of a track exceeds 2^61.
   */
  ...
  /**
   * Finishes writing the contents of the QuickTime output stream without closing the underlying
   * stream. Use this method when applying multiple filters in succession to the same output
   * stream.
   *
   * @throws IllegalStateException if the dimension of the video track has not been specified or
   *                               determined yet.
   ...
   * <p> This method finishes the movie and then copies its content into the specified file. The
   * web-optimized file starts with the movie header.
   *
