...
import static org.bytes2gram.monte.media.BufferFlag.DISCARD;
import static org.bytes2gram.monte.media.BufferFlag.KEYFRAME;
...
import static org.bytes2gram.monte.media.VideoFormatKeys.KeyFrameIntervalKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.MIME_JAVA;
...
import org.bytes2gram.monte.media.AbstractVideoCodec;
import org.bytes2gram.monte.media.Buffer;
import org.bytes2gram.monte.media.Format;
import org.bytes2gram.monte.media.io.ByteArrayImageOutputStream;
 ...
 * <p>
 * The QuickTime player requires that a keyframe is written once per second. This codec enforces
 * this.
 * <p>
 ...
 * </pre>
 * <p>
 * The first 4 bytes defines the chunk length. This field also carries some other unknown flags,
 * since at least one of the high bits is sometimes set.<br>
 * <p>
 * If the overall length of the chunk is less than 8, treat the frame as a NOP, which means that the
 * frame is the same as the one before it.<br>
 * <p>
 * Next, there is a header of either 0x0000 or 0x0008. A header value onlyWith bit 3 set (header
 * &amp; 0x0008) indicates that information follows revealing at which line the decode process is to
 * begin:<br>
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * If the header is 0x0000, then the decode begins from the first line and continues through the
 * entire height of the image.<br>
 * <p>
 * After the header comes the individual RLE-compressed lines. An individual compressed line is
 * comprised of a skip code, followed by a series of RLE codes and pixel data:<br>
 * <pre>
 *  1 byte     skip code
 *  1 byte     RLE code
 *  n bytes    pixel data
 ...
 *  n bytes    pixel data
 * </pre>
 * Each line begins onlyWith a byte that defines the number of pixels to skip in a particular line
 * in the output line before outputting new pixel data. Actually, the skip count is set to one more
 * than the number of pixels to skip. For example, a skip byte of 15 means "skip 14 pixels", while a
 * skip byte of 1 means "don't skip any pixels". If the skip byte is 0, then the frame decode is
 * finished. Therefore, the maximum skip byte value of 255 allows for a maximum of 254 pixels to be
 * skipped.
 * <p>
 * After the skip byte is the first RLE code, which is a single signed byte. The RLE code can have
 ...
 * @version 1.3 2011-01-17 Fixes an index out of bounds exception when a sub-image is compressed.
...
public class AnimationCodec extends AbstractVideoCodec {
  ...
  public AnimationCodec() {
    super(new Format[]{
        ...
        new Format[]{
            ...
            //
            new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_QUICKTIME,
                EncodingKey, ENCODING_QUICKTIME_ANIMATION, DataClassKey, byte[].class, DepthKey,
                16), //
            new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_QUICKTIME,
                EncodingKey, ENCODING_QUICKTIME_ANIMATION, DataClassKey, byte[].class, DepthKey,
                24), //
  ...
  public int process(Buffer in, Buffer out) {
    ...
    boolean isKeyframe = frameCounter == 0
        || frameCounter % outputFormat
        .get(KeyFrameIntervalKey, outputFormat.get(FrameRateKey).intValue()) == 0;
    frameCounter++;
      ...
      switch (vf.get(DepthKey)) {
        case 8: {
          ...
          if (isKeyframe ||//
            ...
            encodeDelta8(tmp, pixels, (byte[]) previousPixels, r.width, r.height,
                r.x + r.y * scanlineStride, scanlineStride);
...
//                        throw new UnsupportedOperationException("Unable to process buffer " + in);
          ...
          if (isKeyframe//
            ...
            encodeDelta16(tmp, pixels, (short[]) previousPixels, r.width, r.height,
                r.x + r.y * scanlineStride, scanlineStride);
            out.setFlag(KEYFRAME, false);
...
//                      throw new UnsupportedOperationException("Unable to process buffer " + in);
          ...
          if (isKeyframe //
            ...
            encodeDelta24(tmp, pixels, (int[]) previousPixels, r.width, r.height,
                r.x + r.y * scanlineStride, scanlineStride);
            out.setFlag(KEYFRAME, false);
          }
          if (previousPixels == null) {
...
//                        return;
          ...
          if (in.isFlag(KEYFRAME) //
            ...
            encodeDelta32(tmp, pixels, (int[]) previousPixels, r.width, r.height,
                r.x + r.y * scanlineStride, scanlineStride);
            out.setFlag(KEYFRAME, false);
          }
          if (previousPixels == null) {
   ...
   */
  public void encodeKey8(ImageOutputStream out, byte[] data, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
    ...
    for (int i = 0, j = 0; i < data.length; i += 4, j++) {
      ints[j] =
          ((data[i] & 0xff) << 24) | ((data[i + 1] & 0xff) << 16) | ((data[i + 2] & 0xff) << 8) | ((
              data[i + 3] & 0xff));
    }
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeDelta8(ImageOutputStream out, byte[] data, byte[] prev, int width, int height,
    ...
    for (int i = 0, j = 0; i < prev.length; i += 4, j++) {
      pints[j] =
          ((prev[i] & 0xff) << 24) | ((prev[i + 1] & 0xff) << 16) | ((prev[i + 2] & 0xff) << 8) | ((
              prev[i + 3] & 0xff));
    }
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeKey16(ImageOutputStream out, short[] data, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeDelta16(ImageOutputStream out, short[] data, short[] prev, int width,
      int height, int offset, int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeKey24(ImageOutputStream out, int[] data, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
    long headerPos = out.getStreamPosition();
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeDelta24(ImageOutputStream out, int[] data, int[] prev, int width, int height,
      int offset, int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeKey32(ImageOutputStream out, int[] data, int width, int height, int offset,
      int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
    long headerPos = out.getStreamPosition();
   ...
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void encodeDelta32(ImageOutputStream out, int[] data, int[] prev, int width, int height,
      int offset, int scanlineStride)
      throws IOException {
    out.setByteOrder(ByteOrder.BIG_ENDIAN);
   ...
   * @param data           The image data.
   * @param prev           The image data of the previous frame. This may be the same object as
   *                       data.
   * @param width          The width of the image in data elements.
   * @param height         The height of the image in data elements.
   * @param offset         The offset to the first pixel in the data array.
   * @param scanlineStride The number to append to offset to get to the next scanline.
   */
  public void decodeDelta16(ImageInputStream in, short[] data, short[] prev, int width, int height,
      int offset, int scanlineStride)
      throws IOException {
    ...
    if (startingLine + numberOfLines - 1 > height) {
      throw new IOException(
          "Illegal startingLine or numberOfLines, startingLine=" + startingLine + ", numberOfLines="
              + numberOfLines);
    }
