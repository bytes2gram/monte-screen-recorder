...
import static org.bytes2gram.monte.media.FormatKeys.MediaType;
 ...
 * QuickTimeOutputStream is not clean. Move write methods in the track classes down to
 * QuickTimeOutputStream.</p>
 *
...
public class AbstractQuickTimeStream {
  ...
  /**
   * The timeScale of the movie. A time value that indicates the time scale for this media—that is,
  ...
  /**
   * Seeks relative to the beginning of the QuickTime stream. <p> Usually this equal to seeking in
   * the underlying ImageOutputStream, but can be different if the underlying stream already
   * contained data.
   */
  ...
  protected abstract static class Group {
    ...
    /**
     * Returns true, if the samples was added to the group. If false is returned, the sample must be
     * added to a new group. <p> A sample can only be added to a group, if the capacity of the group
     * is not exceeded.
     */
    ...
    /**
     * Returns true, if the chunk was added to the group. If false is returned, the chunk must be
     * added to a new group. <p> A chunk can only be added to a group, if the capacity of the group
     * is not exceeded.
     */
  ...
  /**
   * QuickTime stores media data in samples. A sample is a single element in a sequence of
   * time-ordered data. Samples are stored in the mdat atom.
   */
  ...
  protected static class TimeToSampleGroup extends Group {
    ...
    /**
     * Returns true, if the sample was added to the group. If false is returned, the sample must be
     * added to a new group. <p> A sample can only be added to a TimeToSampleGroup, if it has the
     * same duration as previously added samples, and if the capacity of the group is not exceeded.
     */
    @Override
  ...
  protected static class SampleSizeGroup extends Group {
    ...
    /**
     * Returns true, if the sample was added to the group. If false is returned, the sample must be
     * added to a new group. <p> A sample can only be added to a SampleSizeGroup, if it has the same
     * size as previously added samples, and if the capacity of the group is not exceeded.
     */
    @Override
    public boolean maybeAddSample(Sample sample) {
  ...
  /**
   * Groups consecutive samples with the same sample description Id and with adjacent offsets in the
   * movie file.
   */
  protected static class Chunk extends Group {
    ...
    /**
     * Returns true, if the sample was added to the chunk. If false is returned, the sample must be
     * added to a new chunk. <p> A sample can only be added to a chunk, if it has the same sample
     * description Id as previously added samples, if the capacity of the chunk is not exceeded and
     * if the sample offset is adjacent to the last sample in this chunk.
     */
  ...
  /**
   * An {@code Edit} define the portions of the media that are to be used to build up a track for a
   * movie. The edits themselves are stored in an edit list table, which consists of time offset and
   * duration values for each segment. <p> In the absence of an edit list, the presentation of the
   * track starts immediately. An empty edit is used to offset the start time of a track.
   */
  public static class Edit {

    /**
     * A 32-bit integer that specifies the duration of this edit segment in units of the movie's
     * time scale.
     */
    public int trackDuration;
    /**
     * A 32-bit integer containing the start time within the media of this edit segment (in media
     * time scale units). If this field is set to -1, it is an empty edit. The last edit in a track
     * should never be an empty edit. Any differece between the movie's duration and the track's
     * duration is expressed as an implicit empty edit.
     */
    public int mediaTime;
    /**
     * A 32-bit fixed-point number (16.16) that specifies the relative rate at which to play the
     * media corresponding to this edit segment. This rate value cannot be 0 or negative.
     */
     ...
     * @param mediaTime     Start time of this edit in the media's time scale. Specify -1 for an
     *                      empty edit. The last edit in a track should never be an empty edit.
    ...
    /**
     * Creates an edit. <p> Use this constructor only if you want to compute the fixed point media
     * rate by yourself.
     *
     * @param trackDuration Duration of this edit in the movie's time scale.
     * @param mediaTime     Start time of this edit in the media's time scale. Specify -1 for an
     *                      empty edit. The last edit in a track should never be an empty edit.
     * @param mediaRate     The relative rate at which to play this edit given as a 16.16 fixed
     *                      point value.
     */
  ...
  protected class CompositeAtom extends DataAtom {
    ...
    /**
     * Writes the atom and all its children to the ImageOutputStream and disposes of all resources
     * held by the atom.
     *
    ...
    public void finish() throws IOException {
      if (!finished) {
        ...
        DataAtomOutputStream headerData = new DataAtomOutputStream(
            new ImageOutputStreamAdapter(out));
  ...
  protected abstract class Track {
    ...
    /**
     * The timeScale of the media in the track. A time value that indicates the time scale for this
     * media. That is, the number of time units that pass per second in its time coordinate system.
     */
    ...
    /**
     * List of sync samples. This list is null as long as all samples in this track are sync
     * samples.
     */
     ...
     */
    protected void writeTrackAtoms(int trackIndex, CompositeAtom moovAtom, Date modificationTime)
        throws IOException {
      DataAtom leaf;
    ...
    protected abstract void writeMediaInformationHeaderAtom(CompositeAtom minfAtom)
        throws IOException;
  ...
  protected class VideoTrack extends Track {
    ...
    /**
     * Number of bits per ixel. All frames must have the same depth. The value -1 is used to mark
     * unspecified depth.
     */
    protected int videoDepth = -1;
    /**
     * The color table used for rendering the video. This variable is only used when the video uses
    ...
    protected void writeSampleDescriptionAtom(CompositeAtom stblAtom) throws IOException {
      ...
      d.writePString(mediaCompressorName,
          32); // sampleDescriptionTable.videoSampleDescription.compressorName
      ...
      d.writeShort(videoColorTable == null ? -1
          : 0); // sampleDescriptionTable.videoSampleDescription.colorTableID
    ...
    /**
     * Color table atoms define a list of preferred colors for displaying the movie on devices that
     * support only 256 colors. The list may contain up to 256 colors. These optional atoms have a
     * type value of 'ctab'. The color table atom contains a Macintosh color table data structure.
     *
     * @param stblAtom
     * @throws IOException
     */
  ...
  protected class AudioTrack extends Track {
    ...
    /**
     * Sound stsd samples per packet. The number of uncompressed samples generated by a compressed
     * sample (an uncompressed sample is one sample from each channel). This is also the sample
     * duration, expressed in the media’s timescale, where the timescale is equal to the sample
     ...
     * the older sampleSize field, which is set to 16. This value is calculated by dividing the
     * frame size by the number of channels. The same calculation is performed to calculate the
     * value of this field for compressed audio, but the result of the calculation is not generally
     * meaningful for compressed audio.
     */
    protected int soundBytesPerPacket;
    /**
     * The number of bytes in a frame: for uncompressed audio, an uncompressed frame; for compressed
     * audio, a compressed frame. This can be calculated by multiplying the bytes per packet field
     * by the number of channels.
     */
    protected int soundBytesPerFrame;
    /**
     * The size of an uncompressed sample in bytes. This is set to 1 for 8-bit audio, 2 for all
     * other cases, even if the sample size is greater than 2 bytes.
     */
    ...
    /**
     * Extensions to the stsd chunk. Must contain atom-based fields: ([long size, long type, some
     * data], repeat)
     */
