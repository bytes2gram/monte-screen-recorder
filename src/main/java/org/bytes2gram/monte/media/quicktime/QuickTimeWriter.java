 ...
 * <p>
 * Copyright (c) 2010-2011 Werner Randelshofer, Goldau, Switzerland. All rights reserved.
 * <p>
 * You may not use, copy or modify this file, except in compliance with the license agreement you
 * entered into with Werner Randelshofer. For details see accompanying license terms.
 */
...
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_PCM_SIGNED;
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_PCM_UNSIGNED;
...
public class QuickTimeWriter extends QuickTimeOutputStream implements MovieWriter {
  ...
  public final static Format QUICKTIME = new Format(MediaTypeKey, MediaType.FILE, MimeTypeKey,
      MIME_QUICKTIME);
   ...
   */
  @Deprecated
  public int addVideoTrack(Format format, long timeScale, int width, int height)
      throws IOException {
    return addVideoTrack(format.get(EncodingKey), format.get(CompressorNameKey), timeScale, width,
        height, 24, 30);
  }
   ...
   */
  @Deprecated
  public int addVideoTrack(Format format, int width, int height, int depth, int syncInterval)
      throws IOException {
    return addVideoTrack(format.get(EncodingKey), format.get(CompressorNameKey),
        format.get(FrameRateKey).getDenominator() * format.get(FrameRateKey).getNumerator(), width,
        height, depth, syncInterval);
  }
  ...
  public int addAudioTrack(javax.sound.sampled.AudioFormat format) throws IOException {
    ...
    boolean isCompressed =
        format.getProperty("vbr") != null && ((Boolean) format.getProperty("vbr")).booleanValue();
    ...
    if (enc.equals(javax.sound.sampled.AudioFormat.Encoding.ALAW)) {
      ...
      if (sampleSizeInBits != 8) {
        throw new IllegalArgumentException(
            "Sample size of 8 for ALAW required:" + sampleSizeInBits);
      }
      ...
      switch (sampleSizeInBits) {
        ...
        default:
          throw new IllegalArgumentException(
              "Unsupported sample size for PCM_SIGNED:" + sampleSizeInBits);
      }
      ...
      switch (sampleSizeInBits) {
        ...
        default:
          throw new IllegalArgumentException(
              "Unsupported sample size for PCM_UNSIGNED:" + sampleSizeInBits);
      }
    } else if (javax.sound.sampled.AudioFormat.Encoding.ULAW.equals(enc)) {
      if (sampleSizeInBits != 8) {
        throw new IllegalArgumentException(
            "Sample size of 8 for ULAW required:" + sampleSizeInBits);
      }
      qtAudioFormat = "ulaw";
  ...
  private void createCodec(int track) {
    ...
    if (tr.codec != null) {
      if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
        ...
        Format vf = (Format) fmt;
        tr.codec.setInputFormat(fmt.prepend(
            MimeTypeKey, MIME_JAVA, EncodingKey,
            fmt.containsKey(SignedKey) && fmt.get(SignedKey) ? ENCODING_PCM_SIGNED
  ...
  public void write(int track, Buffer buf) throws IOException {
    ...
    // Encode sample data
    {
      ...
      long sampleDurationInMediaTS =
          sampleDuration.getNumerator() * (tr.mediaTimeScale / sampleDuration.getDenominator());
  ...
  public void write(int track, BufferedImage image, long duration) throws IOException {
    ...
    VideoTrack vt = (VideoTrack) tracks
        .get(track); // throws index out of bounds exception if illegal track index
    ...
    if (vt.width == -1) {
      ...
      if (vt.width != image.getWidth() || vt.height != image.getHeight()) {
        throw new IllegalArgumentException(
            "Dimensions of frame[" + tracks.get(track).getSampleCount()
   ...
   */
  @Deprecated
  public void write(int track, byte[] data, int off, int len, long duration, boolean isSync)
      throws IOException {
   ...
   */
  @Deprecated
  public void write(int track, int sampleCount, byte[] data, int off, int len, long sampleDuration,
