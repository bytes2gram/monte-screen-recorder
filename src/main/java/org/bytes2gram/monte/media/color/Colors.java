 ...
 */
package org.bytes2gram.monte.media.color;
...
public class Colors {
  ...
  /**
   * The macintosh palette is arranged as follows: there are 256 colours to allocate, an even
   * distribution of colors through the color cube might be desirable but 256 is not the cube of an
   * integer. 6x6x6 is 216 and so the first 216 colors are an equal 6x6x6 sampling of the color
   * cube. This leaves 40 colours to allocate, this has been done by choosing a ramp of 10 shades
   * each for red, green, blue and grey.
   *
   * <p>
   * References:<br>
  ...
  public static IndexColorModel createMacColors() {
    ...
    // Generate red ramp
    byte[] ramp = {(byte) 238, (byte) 221, (byte) 187, (byte) 170, (byte) 136, (byte) 119, 85, 68,
        34, 17};
    for (int i = 0; i < 10; i++) {
      r[index] = ramp[i];
      g[index] = (byte) (0);
