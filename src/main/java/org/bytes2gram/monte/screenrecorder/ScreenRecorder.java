/*
 * @(#)org.bytes2gram.monte.ScreenRecorder.java
 *
 ...
 */
package org.bytes2gram.monte.screenrecorder;
...
import static org.bytes2gram.monte.media.AudioFormatKeys.ENCODING_QUICKTIME_TWOS_PCM;
...
import static org.bytes2gram.monte.media.AudioFormatKeys.SilenceBugKey;
import static org.bytes2gram.monte.media.AudioFormatKeys.fromAudioFormat;
import static org.bytes2gram.monte.media.BufferFlag.SAME_DATA;
import static org.bytes2gram.monte.media.FormatKeys.EncodingKey;
import static org.bytes2gram.monte.media.FormatKeys.FrameRateKey;
import static org.bytes2gram.monte.media.FormatKeys.MIME_QUICKTIME;
import static org.bytes2gram.monte.media.FormatKeys.MediaTypeKey;
import static org.bytes2gram.monte.media.FormatKeys.MimeTypeKey;
import static org.bytes2gram.monte.media.VideoFormatKeys.COMPRESSOR_NAME_QUICKTIME_ANIMATION;
...
import static org.bytes2gram.monte.media.VideoFormatKeys.ENCODING_BUFFERED_IMAGE;
import static org.bytes2gram.monte.media.VideoFormatKeys.ENCODING_QUICKTIME_ANIMATION;
import static org.bytes2gram.monte.media.VideoFormatKeys.FixedFrameRateKey;
...
import org.bytes2gram.monte.media.AudioFormatKeys;
import org.bytes2gram.monte.media.Buffer;
import org.bytes2gram.monte.media.BufferFlag;
import org.bytes2gram.monte.media.Codec;
import org.bytes2gram.monte.media.Format;
import org.bytes2gram.monte.media.FormatKeys.MediaType;
import org.bytes2gram.monte.media.MovieWriter;
import org.bytes2gram.monte.media.Registry;
import org.bytes2gram.monte.media.avi.AVIWriter;
import org.bytes2gram.monte.media.beans.AbstractStateModel;
import org.bytes2gram.monte.media.color.Colors;
import org.bytes2gram.monte.media.converter.CodecChain;
import org.bytes2gram.monte.media.converter.ScaleImageCodec;
import org.bytes2gram.monte.media.image.Images;
import org.bytes2gram.monte.media.math.Rational;
import org.bytes2gram.monte.media.quicktime.QuickTimeWriter;

/**
 * A screen recorder written in pure Java. <p> Captures the screen, the mouse cursor and audio. <p>
 ...
 * performing in-JVM recordings of an application that is being tested. <p> This recorder uses four
 * threads. Three capture threads for screen, mouse cursor and audio, and one output thread for the
 * movie writer. <p> FIXME - This class is a horrible mess.
 *
 * @author Werner Randelshofer
 * @version $Id: org.bytes2gram.monte.ScreenRecorder.java 303 2013-01-03 07:43:37Z werner $
 */
public class ScreenRecorder extends AbstractStateModel {
   ...
   * @param mouseFormat  The video format for mouse capture. The {@code EncodingKey} must be
   *                     ENCODING_BLACK_CURSOR or ENCODING_WHITE_CURSOR. The {@code SampleRateKey}
   *                     can be independent from the {@code screenFormat}. Specify null if you don't
   *                     want to capture the mouse cursor.
   * @param audioFormat  The audio format for audio capture. Specify null if you don't want audio
   *                     capture.
   */
  ...
  public ScreenRecorder(GraphicsConfiguration cfg,
    ...
    if (this.mouseFormat == null) {
      this.mouseFormat = new Format(FrameRateKey, new Rational(0, 0), EncodingKey,
    ...
    if (mouseFormat != null && mouseFormat.get(FrameRateKey).intValue() > 0) {
      ...
      if (this.mouseFormat.get(EncodingKey).equals(ENCODING_BLACK_CURSOR)) {
        cursorImg = Images
            .toBufferedImage(Images.createImage(ScreenRecorder.class, "images/Cursor.black.png"));
        cursorImgPressed = Images.toBufferedImage(
            Images.createImage(ScreenRecorder.class, "images/Cursor.black.pressed.png"));
      } else if (this.mouseFormat.get(EncodingKey).equals(ENCODING_YELLOW_CURSOR)) {
        cursorImg = Images
            .toBufferedImage(Images.createImage(ScreenRecorder.class, "images/Cursor.yellow.png"));
        cursorImgPressed = Images.toBufferedImage(
            Images.createImage(ScreenRecorder.class, "images/Cursor.yellow.pressed.png"));
      } else {
        cursorImg = Images
            .toBufferedImage(Images.createImage(ScreenRecorder.class, "images/Cursor.white.png"));
        cursorImgPressed = Images.toBufferedImage(
            Images.createImage(ScreenRecorder.class, "images/Cursor.white.pressed.png"));
      }
  ...
  protected MovieWriter createMovieWriter() throws IOException {
    ...
    Rational videoRate = Rational
        .max(screenFormat.get(FrameRateKey), mouseFormat.get(FrameRateKey));
    ...
    if (!videoInputFormat.intersectKeys(WidthKey, HeightKey).matches(
      ...
      sic.setOutputFormat(
          videoOutputFormat.intersectKeys(WidthKey, HeightKey).append(videoInputFormat));
  ...
  /**
   * Creates a file for recording the movie. <p> This implementation creates a file in the users
   * "Video" folder on Windows, or in the users "Movies" folders on Mac OS X. <p> You can override
   * this method, if you would like to create a movie file at a different location.
   *
   * @param fileFormat
   * @return the file
   * @throws IOException
  ...
  protected File createMovieFile(Format fileFormat) throws IOException {
    ...
    File f = new File(movieFolder,//
        "ScreenRecording " + dateFormat.format(new Date()) + "." + Registry.getInstance()
  ...
  private void startScreenCapture() throws AWTException, IOException {
    ...
    screenFuture = screenCaptureTimer
        .scheduleAtFixedRate(screenGrabber, delay, delay, TimeUnit.MILLISECONDS);
  ...
  protected void startMouseCapture() throws IOException {
    ...
    mouseFuture = mouseCaptureTimer
        .scheduleAtFixedRate(mouseGrabber, delay, delay, TimeUnit.MILLISECONDS);
  ...
  /**
   * Waits until mouse capturing stopped. Invoke this method only after you invoked {@link
   * #stopMouseCapture}.
   */
  ...
  private void startAudioCapture() throws LineUnavailableException {
    ...
    int delay = 500;
    audioGrabber = new AudioGrabber(mixer, audioFormat, audioTrack, recordingStartTime,
        writerQueue);
    audioFuture = audioCaptureTimer
        .scheduleWithFixedDelay(audioGrabber, 0, 10, TimeUnit.MILLISECONDS);
  ...
  private void startWriter() {
    writerQueue = new ArrayBlockingQueue<Buffer>(
        max(screenFormat.get(FrameRateKey).intValue(), mouseFormat.get(FrameRateKey).intValue())
            + 1);
  ...
  /**
   * Stops the screen recorder. <p> Stopping the screen recorder may take several seconds, because
   * audio capture uses a large capture buffer. Also, the MovieWriter has to finish up a movie file
   * which may take some time depending on the amount of meta-data that needs to be written.
   */
   ...
   * times, we do this asynchronously. <p> The buffer is copied and passed to the writer queue,
   * which is consumed by the writer thread. See method startWriter(). <p> AVI does not support a
   * variable frame rate for the video track. Since we can not capture frames at a fixed frame rate
   * we have to resend the same captured screen multiple times to the writer. <p> This method is
   * called asynchronously from different threads. <p> You can override this method if you wish to
   * process the media data.
   *
   * @param buf A buffer with un-encoded media data. If {@code buf.track==videoTrack}, then the
   *            buffer contains a {@code BufferedImage} in {@code buffer.data} and a {@code Point}
   *            in {@code buffer.header} with the recorded mouse location. The header is null if the
  ...
  /**
   * The actual writing of the buffer happens here. <p> This method is called exclusively from the
   * writer thread in startWriter().
   *
   * @param buf
   * @throws IOException
   */
  ...
  private static class ScreenGrabber implements Runnable {
    ...
    private void grabScreen() throws IOException, InterruptedException {
      ...
      if (mouseFormat != null && mouseFormat.get(FrameRateKey).intValue() > 0) {
        while (!mouseCaptures.isEmpty()
          ...
          if (mouseCapture.timeStamp.compareTo(prevScreenCaptureTime) > 0) {
            ...
            }
            {
              ...
              if (prevMousePressed) {
                videoGraphics
                    .drawImage(cursorImgPressed, p.x + cursorOffset.x, p.y + cursorOffset.y, null);
              } else {
                videoGraphics
                    .drawImage(cursorImg, p.x + cursorOffset.x, p.y + cursorOffset.y, null);
              }
              buf.clearFlags();
              buf.data = videoImg;
        ...
        if (!hasMouseCapture
            && prevScreenCaptureTime.compareTo(new Rational(getStopTime(), 1000)) < 0) {
          Point p = prevDrawnMouseLocation;
          if (p != null) {
            if (prevMousePressed) {
              videoGraphics
    ...
    public void run() {
      ...
      if (count > 0) {
        ...
        if (buf.timeStamp.add(buf.sampleDuration.multiply(buf.sampleCount)).compareTo(stopTS) > 0) {
          ...
          buf.sampleCount = Math.max(0, (int) Math
              .ceil(stopTS.subtract(buf.timeStamp).divide(buf.sampleDuration).floatValue()));
          buf.length =
              buf.sampleCount * (lineFormat.getSampleSizeInBits() / 8 * lineFormat.getChannels());
