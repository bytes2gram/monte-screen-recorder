...
public class ScreenRecorderCompactMain extends javax.swing.JFrame {
  ...
  public ScreenRecorderCompactMain() {
    ...
    setMinimumSize(
        new Dimension(Math.max(240, getPreferredSize().width), getPreferredSize().height));
    initComponents();
    ...
    Image img = Images
        .createImage(ScreenRecorderCompactMain.class, "images/Recorder.StartStop.png");
    ...
    depth = min(max(0, prefs.getInt("ScreenRecording.colorDepth", 3)),
        colorGroup.getButtonCount() - 1);
    setSelectedIndex(colorGroup, depth);
    format = min(max(0, prefs.getInt("ScreenRecording.format", 1)),
        formatGroup.getButtonCount() - 1);
    ...
    audioRate = min(max(0, prefs.getInt("ScreenRecording.audioRate", 1)),
        audioRateGroup.getButtonCount() - 1);
    setSelectedIndex(audioRateGroup, audioRate);

    channels = min(max(0, prefs.getInt("ScreenRecording.channels", 0)),
        channelsGroup.getButtonCount() - 1);
  ...
  private void updateInfoLabel() {
    ...
    infoLabel.setText(infoLabelText
        .replaceAll("\"Movies\"", "<a href=\"" + movieFolder.toURI() + "\">" + name + "</a>"));
  }
  ...
  private void initComponents() {
    ...
    audioMonitor = new JAudioMonitor();
  ...
  private void start() throws IOException, AWTException {
    ...
    if (screenRecorder == null) {
      ...
      int audioChannels;
      AudioSourceItem asi = (AudioSourceItem) getSelectedItem(audioSourceGroup)
          .getClientProperty("AudioSourceItem");
      ...
      screenRecorder = new ScreenRecorder(cfg, areaRect,
          ...
          movieFolder);
      screenRecorder
          .setAudioMixer(asi.mixerInfo == null ? null : AudioSystem.getMixer(asi.mixerInfo));
  ...
  private void updateTimeLabel() {
    if (screenRecorder != null) {
      ...
      timeLabel.setText(
          (h == 0 ? "" : (h < 10 ? "0" + h : h) + ":") + (m < 10 ? "0" + m : m) + ":" + ((s < 10 ?
              "0" + s : s)));
    }
  }
  ...
  private void selectAreaPerformed(
    ...
    f.addWindowListener(new WindowAdapter() {
      ...
      public void windowClosed(WindowEvent e) {
        ...
        prefs.putInt("org.bytes2gram.monte.ScreenRecorder.customAreaX", customAreaRect.x);
        prefs.putInt("org.bytes2gram.monte.ScreenRecorder.customAreaY", customAreaRect.y);
        prefs.putInt("org.bytes2gram.monte.ScreenRecorder.customAreaWidth", customAreaRect.width);
        prefs.putInt("org.bytes2gram.monte.ScreenRecorder.customAreaHeight", customAreaRect.height);
